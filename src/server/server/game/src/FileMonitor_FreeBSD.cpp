...
void FileMonitorFreeBSD::Update(uint32_t dwPulses)
{
	if( m_KernelEventQueue == INVALID_KERNEL_EVENT || m_FileLists.size() == 0 )
		return;

	int32_t nEvent = kevent(m_KernelEventQueue, &m_TriggeredEventLists[0], (int32_t)m_TriggeredEventLists.size(), &m_MonitoredEventLists[0], (int32_t)m_MonitoredEventLists.size(), NULL );
	...
	else if( nEvent > 0 )
	{
		...
		{
			int32_t nEventFlags = m_MonitoredEventLists[i].flags;
...
void FileMonitorFreeBSD::AddWatch(const std::string& strFileName, PFN_FileChangeListener pListenerFunc)
{
	...
	FileIOContext_FreeBSD context;
	{
		...
		context.idxToEventList = (int32_t)m_MonitoredEventLists.size();
