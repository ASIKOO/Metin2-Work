...
int32_t get_global_time()
{
	return time(0) + global_time_gap;
}

void set_global_time(int32_t t)
{
...
uint32_t str_lower(const char * src, char * dest, uint32_t dest_size)
{
	uint32_t len = 0;
...
const char *one_argument(const char *argument, char *first_arg, uint32_t first_size)
{
	char mark = FALSE;
...
const char *two_arguments(const char *argument, char *first_arg, uint32_t first_size, char *second_arg, uint32_t second_size)
{
	return (one_argument(one_argument(argument, first_arg, first_size), second_arg, second_size));
}

const char *first_cmd(const char *argument, char *first_arg, uint32_t first_arg_size, uint32_t *first_arg_len_result)
{           
	uint32_t cur_len = 0;
	skip_spaces(&argument);
