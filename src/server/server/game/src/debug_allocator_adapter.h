...
struct AllocTag {
	AllocTag(const char* file, uint32_t line) : in_use(true), age(1) {
		this->file = file;
		this->line = line;
	}
	void Reuse(const char* file, uint32_t line) {
		in_use = true;
		this->file = file;
		this->line = line;
		age = IncreaseAge(age);
	}
	void Unuse(const char* file, uint32_t line) {
		in_use = false;
		this->file = file;
		this->line = line;
		age = IncreaseAge(age);
	}
	static uint32_t IncreaseAge(uint32_t value) {
	...
	const char* file;
	uint32_t line;
	uint32_t age; // incremented on both the cases; alloc and free
};
...
public:
	...
	static uint32_t MarkAcquired(void* p, const char* file, uint32_t line, const char* context) {
		if (p == NULL) {
			return 0;
		}
	...
	template<typename T>
	static T* MarkReleased(T* p, const char* file, uint32_t line, const char* context) {
		return (GetInstance().VerifyReference(p, file, line, context, true) ? p : NULL);
	}
    template<typename T>
    static T* MarkReleased(DebugPtr<T>& ptr, const char* file, uint32_t line, const char* context) {
	...
	static uint32_t RetrieveAge(void* p) {
		if (p == NULL) {
			return 0;
		}
		AllocMapType& alloc_map = GetInstance().alloc_map_;
	...
	template<typename T>
	static T* Verify(T* p, uint32_t age, const char* file = NULL, uint32_t line = 0) {
		return (GetInstance().VerifyReference(p, file, line, "ref", false, true, age) ? p : NULL);
	}

	template<typename T>
	static T* VerifyDeletion(T* p, const char* file, uint32_t line, bool verify_age = false, uint32_t age = 0) {
		return (GetInstance().VerifyReference(p, file, line, "pre_delete", false, verify_age, age) ? p : NULL);
	}

	static void LogBoundaryCorruption(void* p, uint32_t age) {
		AllocMapType& alloc_map = GetInstance().alloc_map_;
		AllocMapType::iterator it = alloc_map.find(p);
		if (it != alloc_map.end()) {
			AllocTag& tag = it->second;
...
private:
	...
	bool VerifyReference(void* p, const char* file, uint32_t line,
			const char* context, bool mark_released,
			bool verify_age = false, uint32_t age = 0) {
		if (p == NULL) {
