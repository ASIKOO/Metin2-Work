...
void CGuild::LoadGuildMemberData(SQLMsg* pmsg)
{
	...
	for (uint32_t i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		uint32_t pid = strtoul(row[0], (char**) NULL, 10);
		uint8_t grade = (uint8_t) strtoul(row[1], (char**) NULL, 10);
		uint8_t is_general = 0;

		if (row[2] && *row[2] == '1')
			is_general = 1;

		uint32_t offer = strtoul(row[3], (char**) NULL, 10);
		uint8_t level = (uint8_t)strtoul(row[4], (char**) NULL, 10);
		uint8_t job = (uint8_t)strtoul(row[5], (char**) NULL, 10);
		char * name = row[6];
...
void CGuild::LoadGuildGradeData(SQLMsg* pmsg)
{
	...
	*/
	for (uint32_t i = 0; i < pmsg->Get()->uiNumRows; ++i)
	{
		...
		char * name = row[1];
		uint32_t auth = strtoul(row[2], NULL, 10);
...
void CGuild::LoadGuildData(SQLMsg* pmsg)
{
	...
	m_data.level = (uint8_t)strtoul(row[1], (char **)NULL, 10);
	m_data.exp = strtoul(row[2], (char **)NULL, 10);
	strlcpy(m_data.name, row[3], sizeof(m_data.name));

	m_data.skill_point = (uint8_t) strtoul(row[4], (char **) NULL, 10);
	if (row[5])
...
void CGuild::RefreshCommentForce(uint32_t player_id)
{
	...
	{
		MYSQL_ROW row = mysql_fetch_row(pmsg->Get()->pSQLResult);
		uint32_t id = strtoul(row[0], NULL, 10);
