		...
		if (NULL != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					if ((int32_t)BlueDragon_GetSkillFactor(2, "Skill0", "damage_area") < DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()))
					{
		...
		if (NULL != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					if ((int32_t)BlueDragon_GetSkillFactor(2, "Skill1", "damage_area") < DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()))
					{
						sys_log(0, "BlueDragon: Breath too far (%d)", DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()) );
						return;
					}
		...
		if (NULL != ent)
		{
			if (true == ent->IsType(ENTITY_CHARACTER))
			{
				...
				if (true == ch->IsPC() && false == ch->IsDead())
				{
					...
					if ((int32_t)BlueDragon_GetSkillFactor(2, "Skill2", "damage_area") < DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()))
					{
						sys_log(0, "BlueDragon: Breath too far (%d)", DISTANCE_APPROX(pAttacker->GetX()-ch->GetX(), pAttacker->GetY()-ch->GetY()) );
						return;
					}
