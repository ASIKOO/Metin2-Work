...
public:
	...
	void* Alloc(uint32_t size); // Allocates a memory block
...
inline void* operator new(uint32_t size, const char* file, uint32_t line) {
#ifndef USE_DEBUG_PTR
	void* p = DebugAllocator::Alloc(size);
#else
	void* p = DebugAllocator::Alloc(size + sizeof(uint32_t));
	p = reinterpret_cast<uint32_t*>(p) + 1;
#endif
	if (p != NULL) {
		uint32_t age = DebugAllocator::MarkAcquired(p, file, line, "new");
#ifdef USE_DEBUG_PTR
...
template<typename T>
inline void debug_delete(T* p, const char* file, uint32_t line) {
	if (p != NULL) {
...
#else
		uint32_t* px = reinterpret_cast<uint32_t*>(p) - 1;
...
#ifdef USE_DEBUG_PTR
template<typename T>
inline void debug_delete(DebugPtr<T> ptr, const char* file, uint32_t line) {
	T* p = ptr.Get();
	if (p != NULL) {
...
#endif
template<typename T>
inline void debug_delete_array(T* p, const char* file, uint32_t line) {
	if (p != NULL) {
		DebugAllocator::VerifyDeletion(p, file, line, false);
...
#ifdef USE_DEBUG_PTR
template<typename T>
inline void debug_delete_array(DebugPtr<T> ptr, const char* file, uint32_t line) {
	T* p = ptr.Get();
	if (p != NULL) {
		DebugAllocator::VerifyDeletion(ptr.Get(), file, line, true, ptr.GetAge());
		// Caution: no destructor call here!
...
inline void operator delete(void* p, const char* file, uint32_t line) {
	if (p != NULL) {
		DebugAllocator::VerifyDeletion(p, file, line, false);
#ifndef USE_DEBUG_PTR
		DebugAllocator::Free(p);
