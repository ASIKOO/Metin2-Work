...
bool DESC::Setup(LPFDWATCH _fdw, socket_t _fd, const struct sockaddr_in & c_rSockAddr, uint32_t _handle, uint32_t _handshake)
{
	...
	if (LC_IsEurope())	
	{
		thecore_memcpy(m_adwEncryptionKey, "1234abcd5678efgh", sizeof(uint32_t) * 4);
		thecore_memcpy(m_adwDecryptionKey, "1234abcd5678efgh", sizeof(uint32_t) * 4);
	}
	else
	{
		thecore_memcpy(m_adwEncryptionKey, "testtesttesttest", sizeof(uint32_t) * 4);
		thecore_memcpy(m_adwDecryptionKey, "testtesttesttest", sizeof(uint32_t) * 4);
	}
...
bool DESC::FinishHandshake(uint32_t agreed_length, const void* buffer, size_t length)
{
