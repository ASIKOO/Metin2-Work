...
struct FuncPacketAround
{
	...
	FuncPacketAround(const void * data, int32_t bytes, LPENTITY except = NULL) :m_data(data), m_bytes(bytes), m_except(except)
	{
	}
...
{
	FuncPacketView(const void * data, int32_t bytes, LPENTITY except = NULL) : FuncPacketAround(data, bytes, except)
	{}
