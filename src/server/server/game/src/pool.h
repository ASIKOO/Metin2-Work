...
struct PoolDetail {
	...
	static PointerType Alloc(uint32_t num) {
		return new T[num];
	}
...
class ArrayPool {
public:
	ArrayPool(uint32_t array_size, uint32_t initial_capacity = 0)
			: free_(NULL),
	...
	void Release(T* p) {
		...
		uint32_t index = alloc_index_of_last_release_;
		for (uint32_t i = 0; i < alloc_count_; ++i, ++index) {
			...
			if (begin <= ptr && ptr < end) {
				uint32_t node_index = (ptr - begin) / array_size_;
...
private:
	...
	static const uint32_t kMaxAllocCount = sizeof(uint32_t) * CHAR_BIT;

	bool Stretch(uint32_t increment) {
		if (increment == 0) {
		...
		NodeType* tail = free_;
		NodeType** link = &free_;
		for (uint32_t i = 0; i < increment ; ++i, ++node, p += array_size_) {
			node->block = p;
			*link = node;
			link = &(node->next);
		}
	...
	uint32_t alloc_index_of_last_release_;
...
private:
	typedef std::unordered_map<uint32_t, Pool*> PoolMapType;
	PoolMapType pools_;
};
...
class ObjectPool {
public:
	ObjectPool(uint32_t initial_capacity = 0)
