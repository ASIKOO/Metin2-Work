...
public:
	...
	DebugPtr(T* p, uint32_t age) : p_(p), age_(age) {}
...
struct less<DebugPtr<T> > {
	bool operator()(const DebugPtr<T>& lhs, const DebugPtr<T>& rhs) const {
		return (reinterpret_cast<uint32_t>(lhs.Get()) < reinterpret_cast<uint32_t>(rhs.Get()));
	}
};
template<typename T>
...
struct hash<DebugPtr<T> > {
	uint32_t operator()(const DebugPtr<T>& v) const {
		return reinterpret_cast<uint32_t>(v.Get());
	}
};
#ifdef __GNUC__
}} // namespace std::tr1
