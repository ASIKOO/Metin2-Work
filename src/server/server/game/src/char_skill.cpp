...
int32_t CHARACTER::GetSkillNextReadTime(uint32_t dwVnum) const
{
...
void CHARACTER::SetSkillNextReadTime(uint32_t dwVnum, int32_t time)
{
...
bool CHARACTER::LearnGrandMasterSkill(uint32_t dwSkillVnum)
{
	...
	{
		const int32_t aiGrandMasterSkillBookCountForLevelUp_euckr[10] =
		{
			3, 3, 4, 5, 6, 7, 8, 9, 10, 15, 
		};

		const int32_t aiGrandMasterSkillBookMinCount_euckr[10] = 
		{
			1, 1, 1, 2, 2, 2, 3, 3, 4, 5
		};

		const int32_t aiGrandMasterSkillBookMaxCount_euckr[10] = 
		{
...
bool CHARACTER::LearnSkillByBook(uint32_t dwSkillVnum, uint8_t bProb)
{
	...
	else
	{
		...
		{
			int32_t iBookCount = 99;

			if (LC_IsYMIR() == true)
			{
				const int32_t aiSkillBookCountForLevelUp_euckr[10] =
				{
					2, 2, 3, 3, 3, 3, 3, 3, 4, 5
				};
...
void CHARACTER::ResetSkill()
{
	...
	uint32_t count = sizeof(s_adwSubSkillVnums) / sizeof(s_adwSubSkillVnums[0]);
...
uint32_t CHARACTER::CountMobSkill() const
{
	if (!m_pkMobData)
		return 0;
...
bool CHARACTER::UseMobSkill(uint32_t idx)
{
	...
	for (uint32_t i = 0; i < m_pkMobData->m_mobSkillInfo[idx].vecSplashAttack.size(); i++)
	{
...
bool CHARACTER::CheckSkillHitCount(const uint8_t SkillID, const VID TargetVID)
{
	...
	boost::unordered_map<VID, uint32_t>::iterator iterTargetMap = rSkillUseInfo.TargetVIDMap.find(TargetVID);
