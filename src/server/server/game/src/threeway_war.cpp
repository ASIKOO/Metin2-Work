...
{
	const uint32_t idx = quest::CQuestManager::instance().GetEventFlag( "threeway_war_pass_idx" );
...
{
	const uint32_t idx = quest::CQuestManager::instance().GetEventFlag( "threeway_war_sungzi_idx" );
...
{
	const uint32_t pass_idx = number( 0, PassInfoMap_.size()-1 );
	const uint32_t sung_idx = number( 0, SungZiInfoMap_.size()-1 );
...
{
	boost::unordered_map<uint32_t, uint32_t>::const_iterator iter = RegisterUserMap_.find(PlayerID);
