...
void CClientManager::QUERY_BOOT(CPeer* peer, TPacketGDBoot * p)
{
...
#endif
	int32_t now = time(0);
	peer->Encode(&now, sizeof(int32_t));
	...
	for (uint32_t n = 0; n < vHost.size(); ++n)
	{
	...
	for (uint32_t n = 0; n < vAdmin.size(); ++n)
	{
	...
	uint32_t num_monarch_candidacy = CMonarch::instance().MonarchCandidacySize();
...
void CClientManager::RESULT_PRICELIST_LOAD(CPeer* peer, SQLMsg* pMsg)
{
	...
	uint32_t sizePriceListSize = sizeof(TItemPriceInfo) * header.byCount;
...
void CClientManager::BillingCheck(const char * data)
{
	...
	int32_t curTime = GetCurrentTime();
...
void CClientManager::BlockChat(TPacketBlockChat* p)
{
	...
	{
		MYSQL_ROW row = mysql_fetch_row(pRes->pSQLResult);
		uint32_t pid = strtoul(row[0], NULL, 10);
...
void CClientManager::MyshopPricelistRequest(CPeer* peer, uint32_t dwHandle, uint32_t dwPlayerID)
{
	if (CItemPriceListTableCache* pCache = GetItemPriceListCache(dwPlayerID))
	{
		...
		uint32_t sizePriceListSize = sizeof(TItemPriceInfo) * pTable->byCount;
...
{
	int32_t now = GetCurrentTime();
	ForwardPacket(HEADER_DG_TIME, &now, sizeof(int32_t));
}
...
int32_t CClientManager::GetCurrentTime()
{
	return time(0);
}
...
void CClientManager::EnrollInAuction (CPeer * peer, uint32_t owner_id, AuctionEnrollProductInfo* data)
{
	...
	// 현재 시각 + 24시간 후.
	int32_t expired_time = time(0) + 24 * 60 * 60;
