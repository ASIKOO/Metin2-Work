...
bool CClientManager::InitializeMobTable()
{
	...
	//<파일 읽기>
	cCsvTable nameData;
	if(!nameData.Load("locale/mob_names.txt",'\t'))
	{
	...
	//		(b)[test_map_mobTableByVnum](vnum:TMobTable) 맵을 생성한다.
	//0. 
	set<int32_t> vnumSet;	//테스트용 파일 데이터중, 신규여부 확인에 사용.
	//1. 파일 읽어오기
	...
	while(data.Next()) {
		...
		std::map<uint32_t, TMobTable *>::iterator it_map_mobTable;
...
bool CClientManager::InitializeItemTable()
{
	...
	map<int32_t,const char*> localMap;
	cCsvTable nameData;
	if(!nameData.Load("locale/item_names.txt",'\t'))
	{
	...
	if(!test_data.Load("item_proto_test.txt",'\t'))
	{
		...
		while(test_data.Next()) {
			...
			test_map_itemTableByVnum.insert(std::map<uint32_t, TItemTable *>::value_type(test_item_table->dwVnum, test_item_table));
			test_item_table++;
	...
	while(data.Next()) {
		int32_t vnum = atoi(data.AsStringByIndex(0));
		std::map<uint32_t, TItemTable *>::iterator it_map_itemTable;
	...
	int32_t testValue =  m_vec_itemTable.size();
